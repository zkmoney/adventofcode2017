...
func DayTwo() {
	...
	// 			rowMax = cellInt
	// 		}
	// 		if cellInt < rowMin || j == 1 {
	// 			rowMin = cellInt
	// 		}
	// 	}
	// 	total = total + (rowMax - rowMin)
	// }
	for _, row := range rows {
		total += checksumDigit(row)
	}
	fmt.Println("Checksum:", total)
}

func checksumDigit(stringRow []string) int {
	var intRow []int
	for _, s := range stringRow {
		intRow = append(intRow, toInt(s))
	}
	sort.Sort(sort.Reverse(sort.IntSlice(intRow)))
	for i := 0; i < len(intRow); i++ {
		num := intRow[i]
		for j := 1; j < len(intRow)-i; j++ {
			cellInt := intRow[i+j]
			if num%cellInt == 0 {
				fmt.Println(num / cellInt)
				return num / cellInt
			}
		}
	}
